...
spec = describe "Hasql.Queue.High.AtMostOnce" $ parallel $ do
  sequential $ aroundAll withSetup $ describe "enqueue/dequeue" $ do
    it "enqueue nothing gives nothing" $ withConnection $ \conn -> do
      enqueue schemaName conn E.int4 []
      dequeue schemaName conn D.int4 1 `shouldReturn` []

    it "enqueue 1 gives 1" $ withConnection $ \conn -> do
      enqueue schemaName conn E.int4 [1]
      dequeue schemaName conn D.int4 1 `shouldReturn` [1]
    ...
    it "dequeueing is in FIFO order" $ withConnection $ \conn -> do
      enqueue schemaName conn E.int4 [1]
      enqueue schemaName conn E.int4 [2]
      dequeue schemaName conn D.int4 1 `shouldReturn` [1]
      dequeue schemaName conn D.int4 1 `shouldReturn` [2]

    it "dequeueing a batch of elements works" $ withConnection $ \conn -> do
      enqueue schemaName conn E.int4 [1, 2, 3]
      dequeue schemaName conn D.int4 2 `shouldReturn` [1, 2]

      dequeue schemaName conn D.int4 2 `shouldReturn` [3]
